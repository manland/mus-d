		...
		{	
			if(event.index == 0)
			{
				...
				if(tab_onglet.getOnglet().getCodeMouvementGenerer() && !deja_ouvert_ecouteur)
				{
					...
					(fenetre_code.getTabOngletCode().getChildByName("onglet_mvt") as CanvasCode).getCode().text = tab_onglet.getOnglet().genererClasseEcouteur().toString();
	            ...
	            PopUpManager.addPopUp(fenetre_code, tab_onglet.getOnglet(), false);
